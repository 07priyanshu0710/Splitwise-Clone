...
# Build test database URL based on environment or settings
...
def setup_test_db():
    ...
    # 2. Apply migrations to the test DB using SQLAlchemy directly
    ...
    # Teardown logic if needed (db drops are tricky while connections are open)
...
def db(session_engine):
    ...
    # Cleanup between tests: delete all rows from tables in reverse order to respect foreign keys
...
def test_user_token_headers(client, db):
    # This fixture sets up two users and returns their tokens
    import uuid
