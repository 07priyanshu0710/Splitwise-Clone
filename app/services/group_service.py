...
class GroupService:
    ...
    def create_group(self, group_in: GroupCreate, user_id: int) -> Group:
        # Create group and attribute it to the user
    ...
    def add_member(self, group_id: int, member_in: GroupMemberAdd, requester_id: int):
        ...
        # We allow any member to add other members for now
        
        # Find user by email
        user_to_add = self.user_repository.get_by_email(member_in.email)
        if not user_to_add:
